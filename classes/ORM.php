 ...
 * with object properties representing row data. ORM in Foundation generally follows
 * the Active Record pattern.
 *
 * @link http://wikipedia.org/wiki/Object-relational_mapping
 * @link http://wikipedia.org/wiki/Active_record
 *
 * @package    SuperFan
 * @category   ORM
...
class ORM extends Model implements serializable
{
    ...
    protected function _initialize()
    {
        ...
        if( ! $init = Arr::get( self::$_init_cache, $this->_object_name, FALSE ) )
        {
            $init = array(
                '_belongs_to' => array(),
                '_has_one'    => array(),
    ...
    public function reload_columns( $force = FALSE )
    {
        ...
        {
            if( isset( self::$_column_cache[$this->_object_name] ) )
            {
                // Use cached column information
                $this->_table_columns = self::$_column_cache[$this->_object_name];
            }
            else
            {
                ...
                // Load column cache
                self::$_column_cache[$this->_object_name] = $this->_table_columns;
            }
        }
    ...
    public function with( $target_path )
    {
        ...
        $this
            ->join( array( $target->_table_name, $target_path ), 'LEFT' )
    ...
    public function find()
    {
        if( $this->_loaded )
        {
            throw new Foundation_Exception( 'Method find() cannot be called on loaded objects' );
        }
    ...
    public function find_all()
    {
        if( $this->_loaded )
        {
            throw new Foundation_Exception( 'Method find_all() cannot be called on loaded objects' );
        }
    ...
    protected function _related( $alias )
    {
        ...
        else if( isset( $this->_has_one[$alias] ) )
        {
            return $this->_related[$alias] = self::factory( $this->_has_one[$alias]['model'] );
        }
        else if( isset( $this->_belongs_to[$alias] ) )
        {
