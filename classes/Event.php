...
/**
 * Allows you to register handlers for named events.
 *
 * @package    SuperFan
 * @category   Event
...
class Event
{
    ...
    {
        static::$events[$name][] = array( $once ? 'once' : 'always' => $callback );
    }
    ...
    public static function run( $name, $data = array(), $stop = FALSE )
    {
        if( static::bound( $name ) )
        {
            ...
            foreach( static::$events[$name] as $key => $value )
            {
                ...
                $responses[] = $response = call_user_func_array( $callback, (array) $data );
