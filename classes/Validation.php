...
class Validation implements ArrayAccess
{
    ...
    {
        throw new Foundation_Exception( 'Validation objects are read-only.' );
    }
    ...
    public function check()
    {
        ...
        foreach( $rules as $field => $set )
        {
            ...
            foreach( $set as $array )
            {
                ...
                else if( method_exists( 'Valid', $rule ) )
                {
    ...
    public function errors( $file = NULL )
    {
        ...
        foreach( $this->_errors as $field => $set )
        {
            ...
            if( $message = Foundation::message( $file, "{$field}.{$error}" )  && is_string( $message ) )
            {
                // Found a message for this field and error
            }
            else if( $message = Foundation::message( $file, "{$field}.default" ) && is_string( $message ) )
            {
                // Found a default message for this field
            }
            else if( $message = Foundation::message( $file, $error ) && is_string( $message ) )
            {
                // Found a default message for this error
            }
            else if( $message = Foundation::message( 'validation', $error ) && is_string( $message ) )
            {
                // Found a default message for this error
            }
            else
