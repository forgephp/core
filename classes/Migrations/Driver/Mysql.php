...
class Migrations_Driver_MySQL extends Migrations_Driver
{
	...
	public function create_table( $table_name, $fields, $primary_key = TRUE )
	{
		...
		foreach( $fields as $field_name => $params )
		{
			...
			if( $primary_key === $field_name && $params[0] == 'integer' )
			{
				$params['auto'] = TRUE;
			}
	...
	public function add_index( $table_name, $index_name, $columns, $index_type = 'normal' )
	{
		switch( $index_type )
		{
			...
			default:
				throw new Foundation_Exception( 'migrations.bad_index_type :index_type', array( ':index_type' => $index_type ) );
		}
	...
	protected function compile_column( $field_name, $params, $allow_order = FALSE )
	{
		if( empty( $params ) )
		{
			throw new Foundation_Exception( 'Migrations::compile_column missing_argument $params' );
		}
		...
		foreach( $params as $key => $param )
		{
			...
			if( is_string( $key ) )
			{
				switch( $key )
				{
					...
					default:
						throw new Foundation_Exception( 'Migrations::compile_column - bad_column :key', array( ':key' => $key ) );
				}
			...
			throw new Foundation_Exception( 'Migrations::compile_column - bad_column :column', array( ':column' => $column ) );
		}

		if( empty( $type ) )
		{
			throw new Foundation_Exception( 'Migrations::compile_column - missing_argument $type' );
		}
	...
	protected function get_column( $table_name, $column_name )
	{
		...
		if( $result->count() !== 1 )
		{
			throw new Foundation_Exception( 'Migrations::get_column - column_not_found :col_name, :table_name',
	...
	protected function native_type( $type, $limit )
	{
		...
		{
			throw new Foundation_Exception( 'Migrations::native_type - unknown_type :type', array( ':type' => $type ) );
		}
	...
	protected function migration_type( $native )
	{
		...
		{
			throw new Foundation_Exception( 'Migrations::migration_type - unknown_type :type', array( ':type' => $native ) );
		}
