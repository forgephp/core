...
class Request implements HTTP_Request
{
    ...
    public static function factory( $uri = TRUE , $client_params = array(), $injected_routes = array() )
    {
        ...
        if ( ! self::$initial )
        {
            ...
            if( (
                ...
                && in_array( $_SERVER['REMOTE_ADDR'], self::$trusted_proxies )
            ){
            ...
            self::$initial = $request = new Request( $uri, $client_params, $injected_routes);
    ...
    public static function detect_uri()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                throw new Foundation_Exception( 'Unable to detect the URI using PATH_INFO, REQUEST_URI, PHP_SELF or REDIRECT_URL' );
            }
    ...
    public static function post_max_size_exceeded()
    {
        ...
        if( self::$initial->method() !== self::POST )
        {
            return FALSE;
        }
    ...
    public static function process( Request $request, $routes = NULL )
    {
        ...
        foreach( $routes as $route )
        {
            ...
            if( $params = $route->matches( $request, $request->method() ) )
            {
                return array(
                    'route'  => $route,
                    'params' => $params,
    ...
    public function execute()
    {
        ...
        if( ! $this->_route instanceof Route )
        {
            ...
            )->request( $this )->get_response();
        }

        if( ! $this->_client instanceof Request_Client )
        {
            throw new Request_Exception(
                'Unable to execute :uri without a Request_Client',
                array(
                    ':uri' => $this->_uri,
                )
            );
    ...
    /**
     * Returns whether this request is the initial request Foundation received.
     ...
     *  - Body
     *
     *  If there are variables set to the `Foundation_Request::$_post`
    ...
    public function render()
    {
        ...
        {
            $this->headers('user-agent', Foundation::version());
        }
