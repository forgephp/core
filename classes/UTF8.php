...
class UTF8
{
    ...
    protected static $utf8_upper_to_lower = NULL;

    protected static $utf8_lower_accents = NULL;

    protected static $utf8_upper_accents = NULL;

    protected static $utf8_lower_to_upper = NULL;
     ...
     * @param   string  $charset    character set, defaults to Foundation::$charset
     * @return  mixed
     * @uses    UTF8::clean
    ...
    public static function transliterate_to_ascii( $str, $case = 0 )
    {
        if( $case <= 0 )
        {
            if( self::$utf8_lower_accents === NULL )
            {
                self::$utf8_lower_accents = array(
            ...
            $str = str_replace(
                array_keys( self::$utf8_lower_accents ),
                array_values( self::$utf8_lower_accents ),
                $str
            );
        }
        ...
        if( $case >= 0 )
        {
            if( self::$utf8_upper_accents === NULL )
            {
                self::$utf8_upper_accents = array(
                    'À' => 'A',  'Ô' => 'O',  'Ď' => 'D',  'Ḟ' => 'F',  'Ë' => 'E',  'Š' => 'S',  'Ơ' => 'O',
            ...
            $str = str_replace(
                array_keys( self::$utf8_upper_accents ),
                array_values( self::$utf8_upper_accents ),
                $str
            );
        }
     ...
     * @uses    Foundation::$charset
     */
    public static function strlen( $str )
    {
        if( self::$server_utf8 )
        {
            return mb_strlen( $str, Foundation::$charset );
        }
    ...
    public static function strpos($str, $search, $offset = 0)
    {
        if( self::$server_utf8 )
        {
            return mb_strpos( $str, $search, $offset, Foundation::$charset );
        }
    ...
    public static function strrpos( $str, $search, $offset = 0 )
    {
        if( self::$server_utf8 )
        {
            return mb_strrpos( $str, $search, $offset, Foundation::$charset );
        }
    ...
    public static function substr($str, $offset, $length = NULL)
    {
        ...
        {
            return ( $length === NULL )
                ? mb_substr( $str, $offset, mb_strlen( $str ), Foundation::$charset )
                : mb_substr( $str, $offset, $length, Foundation::$charset )
            ;
        }
    ...
    public static function strtolower($str)
    {
        if( self::$server_utf8 )
        {
            return mb_strtolower( $str, Foundation::$charset );
        }
        ...
        if( self::$utf8_upper_to_lower === NULL )
        {
            self::$utf8_upper_to_lower = array(
        ...
        {
            if( isset( self::$utf8_upper_to_lower[$uni[$i]] ) )
            {
                $uni[$i] = self::$utf8_upper_to_lower[$uni[$i]];
            }
        }
    ...
    public static function strtoupper($str)
    {
        if( self::$server_utf8 )
        {
            return mb_strtoupper( $str, Foundation::$charset );
        }
        ...
        if( self::$utf8_lower_to_upper === NULL )
        {
            self::$utf8_lower_to_upper = array(
        ...
        for( $i = 0, $c = count( $uni ); $i < $c; $i++ )
        {
            if( isset( self::$utf8_lower_to_upper[$uni[$i]] ) )
            {
    ...
    public static function str_pad($str, $final_str_length, $pad_str = ' ', $pad_type = STR_PAD_RIGHT)
    {
        ...
        throw new Foundation_Exception( "UTF8::str_pad: Unknown padding type (:pad_type)", array( ':pad_type' => $pad_type ) );
    }
    ...
    public static function ord( $chr )
    {
        ...
        if( ! isset( $chr[1] ) )
        {
            throw new Foundation_Exception( 'UTF8::ord: Short sequence - at least 2 bytes expected, only 1 seen' );
        }
        ...
        if( ! isset( $chr[2] ) )
        {
            throw new Foundation_Exception('UTF8::ord: Short sequence - at least 3 bytes expected, only 2 seen');
        }
        ...
        if( ! isset( $chr[3] ) )
        {
            throw new Foundation_Exception('UTF8::ord: Short sequence - at least 4 bytes expected, only 3 seen');
        }
        ...
        if( ! isset( $chr[4] ) )
        {
            throw new Foundation_Exception('UTF8::ord: Short sequence - at least 5 bytes expected, only 4 seen');
        }
        ...
        if( ! isset( $chr[5] ) )
        {
            throw new Foundation_Exception('UTF8::ord: Short sequence - at least 6 bytes expected, only 5 seen');
        }
        ...
        {
            throw new Foundation_Exception("UTF8::ord: Invalid UTF-8 with surrogate ordinal ':ordinal'", array(
                ':ordinal' => $ord0,
            ));
        }
    }
        ...
        for( $i = 0; $i < $len; $i++ )
        {
            ...
            if( $m_state == 0 )
            {
                ...
                else
                {
                    ...
                    throw new Foundation_Exception( 'UTF8::to_unicode: Illegal sequence identifier in UTF-8 at byte :byte', array( ':byte' => $i ) );
                }
            }
            else
            {
                ...
                if( 0x80 == ( 0xC0 & $in ) )
                {
                    ...
                    if( 0 == --$m_state )
                    {
                        ...
                        if (((2 == $m_bytes) && ($m_ucs4 < 0x0080))  ||
                            ...
                            throw new Foundation_Exception( 'UTF8::to_unicode: Illegal sequence or codepoint in UTF-8 at byte :byte', array( ':byte' => $i ) );
                        }
                ...
                else
                {
                    ...
                    throw new Foundation_Exception( "UTF8::to_unicode: Incomplete multi-octet sequence in UTF-8 at byte ':byte'", array(
                        ':byte' => $i,
                    ));
                }
            }
    ...
    public static function from_unicode( $arr )
    {
        ...
        foreach( $keys as $k )
        {
            ...
            {
                // Found a surrogate
                throw new Foundation_Exception( "UTF8::from_unicode: Illegal surrogate at index: ':index', value: ':value'", array(
                    ':index' => $k,
                    ':value' => $arr[$k],
                ));
            }
            ...
            {
                throw new Foundation_Exception("UTF8::from_unicode: Codepoint out of Unicode range at index: ':index', value: ':value'", array(
                    ':index' => $k,
                    ':value' => $arr[$k],
                ));
            }
