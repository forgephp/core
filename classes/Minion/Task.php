...
abstract class Minion_Task
{
	...
	public static function convert_class_to_task( $class )
	{
		...
		return strtolower( str_replace( '_', self::$task_separator, substr( $class, 5 ) ) );
	}
	...
	public static function factory( $options )
	{
		...
		$class = self::convert_task_to_class_name( $task );
	...
	public function __toString()
	{
		...
		if( $task_name === NULL )
		{
			$task_name = self::convert_class_to_task( $this );
		}
	...
	public function execute()
	{
		...
		{
			echo View::factory( 'minion/validation' )
				->set( 'task', self::convert_class_to_task( $this ) )
	...
	protected function _compile_task_list( array $files, $prefix = '' )
	{
		...
		foreach( $files as $file => $path )
		{
			...
			{
				$task = $this->_compile_task_list( $path, $prefix . $file . self::$task_separator );
