...
class HTTP_Header extends ArrayObject
{
     ...
     *
     *     $header_object = new HTTP_Header( array( 'x-powered-by' => 'Foundation 6.0.x', 'expires' => '...' ) );
     *
     ...
     * request Accept: header. If the $explicit argument is TRUE, only
     * precise matches will be returned, excluding all wildcard (`*`) directives.
     *
    ...
    public function accepts_at_quality( $type, $explicit=FALSE )
    {
        ...
        {
            $mime = Foundation::$config->load( 'mimes.' . $type );
        ...
        else
        {
            ...
            else if( isset( $this->_accept_content['*']['*'] ) )
            {
    ...
    public function accepts_charset_at_quality( $charset )
    {
        ...
        else if( isset( $this->_accept_charset['*'] ) )
        {
    ...
    public function accepts_language_at_quality( $language, $explicit=FALSE )
    {
        ...
        if( isset( $this->_accept_language[$language_parts[0]] ) )
        {
            if( isset( $language_parts[1] ) )
            {
                ...
                else if( $explicit === FALSE && isset( $this->_accept_language[$language_parts[0]]['*'] ) )
                {
                    return $this->_accept_language[$language_parts[0]]['*'];
                }
            }
            else if( isset( $this->_accept_language[$language_parts[0]]['*'] ) )
            {
                return $this->_accept_language[$language_parts[0]]['*'];
            }
        }
    ...
    public function send_headers( HTTP_Response $response=NULL, $replace=FALSE, $callback=NULL )
    {
        ...
        {
            $processed_headers[] = 'Content-Type: ' . Foundation::$content_type . '; charset=' . Foundation::$charset;
        }

        if( Foundation::$expose && ! isset( $headers['x-powered-by'] ) )
        {
            $processed_headers[] = 'X-Powered-By: ' . Foundation::version();
        }
