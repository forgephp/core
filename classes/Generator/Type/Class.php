 ...
 */
class Generator_Type_Class extends Generator_Type
{
	...
	public function render()
	{
		...
		if( empty( $methods ) && empty( $this->_params['abstract'] ) && empty( $this->_params['blank'] ) )
		{
			$implemented = array();
			...
			if( ! empty( $this->_params['extends'] ) )
			{
				...
				$implemented = $this->_get_reflection_methods( $this->_params['extends'], Generator_Reflector::TYPE_CLASS, TRUE );
			}

			if( ! empty( $this->_params['traits'] ) )
			{
				// Implement any trait's abstract methods
				$implemented += $this->_get_reflection_methods( $this->_params['traits'], Generator_Reflector::TYPE_TRAIT, TRUE );
			}

			if( ! empty( $this->_params['implements'] ) )
			{
				// Implement any interface methods
				$implemented += $this->_get_reflection_methods( $this->_params['implements'], Generator_Reflector::TYPE_INTERFACE, TRUE );
			}
	 ...
	 */
	protected function _get_reflection_methods( $sources, $type, $inherit = FALSE, Generator_Reflector $reflector = NULL )
	{
			...
			if( $refl->source( $source )->exists() )
			{
				foreach( $refl->get_methods( $implementing, $inherit ) as $method => $m )
				{
					...
					if( empty( $m['doccomment'] ) )
					{
						...
						$doc = View::factory( 'generator/type/doccomment')
							->set( 'tabs', 1 )
						;
					...
					if( ! $m['abstract'] && $m['inherited'] && empty( $m['made_concrete'] ) )
					{
						// Invoke the parent for inherited methods
						$m['body']  = isset( $doc ) ? '' : ( '// Defined in ' . $m['class'] . PHP_EOL . "\t\t" );
						$m['body'] .= ( isset( $doc ) || ! preg_match( '/@return\s+void/i', $m['doccomment'] ) ) ? 'return ' : '';
						$m['body'] .= 'parent::' . $refl->get_method_invocation( $method ) . ';';
					}
					elseif( ! $m['abstract'] )
					{
						// Otherwise just add an in-line comment
						$m['body'] = isset( $doc ) ? '// Method implementation' : "// Implementation of {$m['class']}::{$method}";
					}
	...
	protected function _group_by_modifier( array $source )
	{
		...
		foreach( $source as $key => $value )
		{
			...
			else if( strpos( $value['modifiers'], 'abstract' ) !== FALSE )
			{
				$grouped['abstract'][$key] = $value;
			}
			else if( strpos( $value['modifiers'], 'public' ) !== FALSE )
			{
