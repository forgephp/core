 ...
 */
class Generator_Type_Clone extends Generator_Type_Class
{
	...
	public function render()
	{
		...
		if( ! isset( $this->_params['blank'] ) )
		{
			...
			$this->_params['constants'] = $this->_get_reflection_constants( $source, $type, $refl );
			...
			$this->_params['methods'] = $this->_get_reflection_methods( $source, $type, $this->_inherit, $refl );
		}
		...
		foreach( (array) $sources as $source )
		{
			...
			if( $refl->source( $source )->exists() )
			{
				foreach( $refl->get_constants() as $constant => $c )
				{
					...
					if( ! $this->_inherit && $c['class'] != $source )
					{
						continue;
					}
		...
		foreach( (array) $sources as $source )
		{
			...
			if( $refl->source($source)->exists() )
			{
				foreach( $refl->get_properties() as $property => $p )
				{
					...
					if( ! $this->_inherit && $p['class'] != $source )
					{
						continue;
					}
					...
					if( empty( $p['doccomment'] ) )
					{
						...
						$doc = View::factory( 'generator/type/doccomment' )
							->set( 'tabs', 1 )
							->set( 'short_description',  "Declared in {$p['class']}" )
							->set( 'tags', '@var  ' . $p['type'] . '  $' . $property )
						;
